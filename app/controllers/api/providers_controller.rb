module Api
  class ProvidersController < ApiController
    ...
    skip_before_action :check_parent_credentials, only: %w[show store_email]

    def store_email
      temp_parent = TempParent.where(email: params[:email]).first_or_create
      session[:api_key] = temp_parent.api_key
