...
// FareHarbor additions:
//
//  Bug fixes:
//    On IE9 at least, if you don't handle and `preventDefault` / `stopPropagation`
//    on clicks on anchors (either via `ng-click` or by hand), you get an error
//    when clicking the anchor.
//
//  Static interpolation:
//    We have a custom implementation of `$interpolate` that introduces "static"
//    interpolations. To get the most performance benefit we also tweak Angular
//    to understand static interpolations and avoid introducing watches.  See
//    `addTextInterpolateDirective` and `addAttrInterpolateDirective`.
//
//  Sexier URLs:
//    We tweak `encodeUriQuery` to allow / in query parameter values; this is
//    against the spec but works everywhere and looks nice.
//
//  To compile our tweaked build use the Google Closure Compiler:
//
//    $ closure-compiler --language_in=ECMASCRIPT5 --js=angular.js > angular.min.js
//
      ...
      if (interpolateFn) {
        directives.push({
          ...
          compile: valueFn(function textInterpolateLinkFn(scope, node) {
            // FH: Avoid unneeded watches in static interpolations; depends on our overridden `$interpolate`.
            var interpolateFn = $interpolate(text, true);
            var value = interpolateFn(scope);
            
            node[0].nodeValue = value;
            if(typeof interpolateFn.value !== 'undefined'){
              return;
            }
  ...
  this.$get = ['$rootScope', '$browser', '$sniffer', '$rootElement',
    ...
    $rootElement.on('click', function(event) {
      ...
      if (LocationMode === LocationHashbangInHtml5Url) {
        ...
        // FH: Angular bug in old IE when there is not `href` attribute; fix by checking
        // for `isString` first.
        if (isString(href) && href.indexOf('://') < 0) {         // Ignore absolute URLs
